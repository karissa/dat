...
var auth = require('../../cookie-auth/auth.js')
...
function RestHandler(dat) {
  ...
  this.dat = dat
  user = dat.options.adminUser || process.env["DAT_ADMIN_USER"]
  pass = dat.options.adminPass || process.env["DAT_ADMIN_PASS"]
  this.auth = auth(user, pass)
